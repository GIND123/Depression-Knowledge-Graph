...
While the integration of Knowledge Graphs (KGs) with Large Language Models is established in medical AI, current methodologies predominantly treat KGs as static references rather than dynamic, evolving structures. Standard approaches, such as those combining RLHF with KG guidance [@Wangref1] or utilizing GraphRAG for symptom verification [@Guoref2], rely on fixed standards like DSM-5 without real-time structural evolution. Extraction-focused methods further limit adaptability; MedKG [@Linref3] and Text-to-GraphQL systems [@Niref4] depend on rigid schemas and pre-constructed repositories that fail to capture the nuances of ongoing dialogue. Most temporal systems also present challenges for personalized care: while some autonomous agents track the evolution of scientific literature [@zhangref5], they ignore real-time user states. Consequently, existing diagnostic frameworks [@yuan6] remain tethered to general clinical rules, lacking the dynamic memory required to interpret unique, unfolding patient contexts.
